...
namespace DoAn4.Repositories
{
    public class FriendshipRepository : IFriendshipRepository
    {
        ...
        .       FirstOrDefaultAsync(f => f.UserId == sender && f.FriendUserId == receiver || f.UserId == receiver && f.FriendUserId == receiver);
            return friendship;
        }
        ...
        public async Task<List<FriendshipDto>> GetAllFriendshipPendingAsync(Guid currentUserId)
        {
            var friendshipPending = await _context.Friendships
                .Where(f => (f.UserId == currentUserId || f.FriendUserId == currentUserId) && f.FriendStatus == 0)
                .ToListAsync();
        ...
        public async Task<List<FriendshipDto>> GetAllFriendshipAsync(Guid currentUserId)
        {
            ...
            return _mapper.Map<List<FriendshipDto>>(friendship);
        }
        ...
        public async Task<List<Guid>> GetAllFriendUseridFromListFriendshipAsync(List<FriendshipDto> friendships)
        {
            return  friendships.Select(f => f.FriendUserId).ToList();
        }
